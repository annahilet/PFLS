...
echo "Total length of sequences:" $total_length
    ...
    longest_seq=$(awk '/>/ {if (seq) print seq; print; seq=""; next} {seq=seq $0} END {print seq}' $1 |awk '!/>/{print}' | awk '{ print length, $0 }' | sort -nr |head -n 1 | awk '{print $1}')
echo "Length of the longest sequence:" $longest_seq

    shortest_seq=$(awk '/>/ {if (seq) print seq; print; seq=""; next} {seq=seq $0} END {print seq}' $1 |awk '!/>/{print}' | awk '{ print length, $0 }' | sort -n |head -n 1 | awk '{print $1}')
